...
namespace datacoe
{
    ...
    TEST_F(IntegrationTest, FullLifecycle)
    {
        ...
        {
            // 1. Create game data
            std::array<std::size_t, 4> originalScores = {1000, 2000, 3000, 4000};
            GameData originalData("IntegrationTest", originalScores);
            ...
            ASSERT_EQ(loadedData.getHighscores(), originalScores);
            ...
            ASSERT_EQ(readData.value().getHighscores(), updatedScores);
        }
    ...
    TEST_F(IntegrationTest, MultipleInstances)
    {
        try
        {
            ...
            data1.setHighscores(initialScores);
            ...
            ASSERT_EQ(dm3.getGamedata().getHighscores(), updatedScores);

            // Original instance should still have old data in memory
            ASSERT_EQ(dm1.getGamedata().getHighscores(), initialScores);

            // After reloading, it should see the new data
            dm1.loadGame();
            ASSERT_EQ(dm1.getGamedata().getHighscores(), updatedScores);
        }
        catch (const std::exception &e)
        {
            FAIL() << "Unexpected exception: " << e.what();
    ...
    TEST_F(IntegrationTest, DataCorruption)
    {
        try
        {
            ...
            ASSERT_EQ(dm2.getGamedata().getHighscores(), (std::array<std::size_t, 4>{0, 0, 0, 0}));

            // Save new data
            GameData recoveredData;
            recoveredData.setNickname("RecoveredData");
            std::array<std::size_t, 4> newScores = {300, 400, 500, 600};
            recoveredData.setHighscores(newScores);
            dm2.setGamedata(recoveredData);
            ASSERT_TRUE(dm2.saveGame());
            ...
            ASSERT_EQ(dm3.getGamedata().getHighscores(), newScores);
        }
        catch (const std::exception &e)
        {
            FAIL() << "Unexpected exception: " << e.what();
